...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET || 'fallback_secret', (err, admin) => {
    if (err) {
    ...
    const mailOptions = {
      ...
      html: `
        ...
        <head>
            ...
            <title>Admin Registration Confirmation</title>
            <style>
...
// Update video
app.put('/api/videos/:id', authenticateToken, upload.single('videoFile'), async (req, res) => {
  try {
...
// Update question
app.put('/api/questions/:id', authenticateToken, upload.fields([
  { name: 'questionImage', maxCount: 1 },
  { name: 'questionFile', maxCount: 1 }
]), async (req, res) => {
  try {
...
// Update paper
app.put('/api/papers/:id', authenticateToken, upload.single('paperFile'), async (req, res) => {
  try {
...
// Update sports media
app.put('/api/sports/:id', authenticateToken, upload.single('sportsMedia'), async (req, res) => {
  try {
...
app.put('/api/users/profile', authenticateUser, upload.single('profileImage'), async (req, res) => {
  try {
    ...
    const { fullName, phoneContact, school } = req.body;
